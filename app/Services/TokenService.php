...
class TokenService
{
     ...
     * Save token to DB
     */
    public function saveToken(string $token, int $userId, Time $expires, string $type, bool $blacklisted = false): void
    {
    ...
    /**
     * Verify token signature and DB existence
     */
    public function verifyToken(string $token, string $type): Token
    {
        ...
        if (!$tokenDoc) {
            throw new Exception('Token not found or invalidated');
        }
    ...
    public function generateResetPasswordToken(string $email): string
    {
        ...
        $token = $this->generateToken($user->id, $expires, $this->config->tokenTypes['RESET_PASSWORD']);
        
        $this->saveToken($token, $user->id, $expires, $this->config->tokenTypes['RESET_PASSWORD']);
        return $token;
    }
    ...
    public function generateVerifyEmailToken(User $user): string
    {
        ...
        $token = $this->generateToken($user->id, $expires, $this->config->tokenTypes['VERIFY_EMAIL']);

        $this->saveToken($token, $user->id, $expires, $this->config->tokenTypes['VERIFY_EMAIL']);
        return $token;
    }
}